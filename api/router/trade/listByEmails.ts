...
export default async function listByEmails(
  ...
  try {
    ...
    const expansionIds = new Set<string>(collections.map((collection) => collection.expansionId))
    ...
    for (const expansionId of expansionIds) {
      ...
      for (const otherUserExpansionCollection of otherUserExpansionCollections) {
        const collectionTrades = getTrades(userExpansionCollection, otherUserExpansionCollection)
