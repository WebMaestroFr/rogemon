...
export function getTradeCard(cardId: string, collection: ICollection): ITradeCard {
  return {
...
export function getTrade(userTradeCard: ITradeCard, otherTradeCard: ITradeCard): ITrade {
...
export function getTrades(userCollection: ICollection, otherCollection: ICollection): ITrade[] {
  const cardIds = new Set([...userCollection.countMap.keys(), ...otherCollection.countMap.keys()])
  ...
  for (const userTradeCard of userDuplicateOtherMissing) {
    const userCardRarity = getCardRarity(userTradeCard.expansionId, userTradeCard.cardId)
    for (const otherTradeCard of userMissingOtherDuplicate) {
      const otherCardRarity = getCardRarity(otherTradeCard.expansionId, otherTradeCard.cardId)
